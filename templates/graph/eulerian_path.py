...
from custom_type import GRAPH, LIST1D, TREE
...
class EulerianPath:
    ...
    def get_Eulerian_path(self):
        if all([self._in[v] == self._out[v] for v in self._in.keys()]):
            ...
            self.dfs([v for v in self._in.keys()
                     if self._in[v] + 1 == self._out[v]][0])
            return self.path[::-1]
        else:
...
if __name__ == "__main__":
    assert EulerianPath({0: {1: 1}, 1: {2: 1}, 2: {}},
                        True).get_Eulerian_path() == [0, 1, 2]

    g = defaultdict(dict)
    g[0][1] = g[1][0] = g[1][2] = g[2][1] = 1
    assert get_Eulerian_path(g) == [2, 1, 0]

    start, end = eulerian_technique([[2, 3, 4], [], [], [1], []])
    assert end[0]-start[0] == 4
    assert end[1]-start[1] == 0
    assert end[2]-start[2] == 0
    assert end[4]-start[4] == 0
