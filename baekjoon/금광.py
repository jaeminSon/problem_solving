...
l = [[int(el) for el in sys.stdin.readline().rstrip().split()]
     for _ in range(N)]
...
unique_y_sorted = sorted(set([mapping[0][el[0]] for el in l]))
line = defaultdict(list)
for el in l:
    line[mapping[0][el[0]]].append((mapping[1][el[1]], el[2]))

segtree = BestPrefixSumSegmentTree(4096)
ans = 0
