...
tree_downward = [0] * 2 * just_bigger_power_2(n)
lazy_downward = [0] * 2 * just_bigger_power_2(n)
tree_upward = [0] * 2 * just_bigger_power_2(n)
lazy_upward = [0] * 2 * just_bigger_power_2(n)
...
initialize(tree_downward, [0]*n, 0, n-1, 0)
initialize(tree_upward, [0]*n, 0, n-1, 0)
...
for q in queries:
    if q[0] == 1:
        node = q[1] - 1
        if downward:
            update(tree_downward, lazy_downward, start[node], end[node], q[2])
        else:
            update(tree_upward, lazy_upward, start[node], start[node], q[2])
    elif q[0] == 2:
        node = q[1]-1
        print(query(tree_downward, lazy_downward, start[node], start[node]) + 
              query(tree_upward, lazy_upward, start[node], end[node]))
    else:
